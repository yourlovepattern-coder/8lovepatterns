/* ============================================================================
   LovePattern  .  /api/stripe-webhook   (Netlify Function)   THE ROBOT
   ----------------------------------------------------------------------------
   ...
   Why a link, not a PDF: the report is now assembled server-side and gated. The
   browser never holds the content before payment, so there is no pre-built PDF
   to attach. The buyer prints to PDF from the report page in one click.
...
export default async (req) => {
  ...
  try {
    const order = token ? await getStore('orders').get(token, { type: 'json', consistency: 'strong' }) : null;
    if (order) {
  ...
  const sent = await sendWithResend({ to: email, subject: mail.subject, html: mail.html, text: mail.text });
...
async function sendWithResend({ to, subject, html, text }) {
  ...
  const from = process.env.MAIL_FROM || 'LovePattern <support@8lovepatterns.com>';
  try {
